# Issue #16 — Migrate `docx` connector to the canonical contract
...
Migrate the `docx` connector onto the canonical contract per the plan's *Architectural
decisions* and Phase 6. Using the shared node-mapping helper: emit a `CompositionNode`
tree of canonical atoms (Text, Table, Image), return a `Result`, set a semantic `kind`,
