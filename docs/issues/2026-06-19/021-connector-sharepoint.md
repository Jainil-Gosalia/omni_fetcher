# Issue #21 — Migrate `sharepoint` connector to the canonical contract
...
Migrate the `sharepoint` connector onto the canonical contract per the plan's
*Architectural decisions* and Phase 7. Using the shared node-mapping helper: emit a
`CompositionNode` tree of canonical atoms (sites/libraries as container nodes, files as
...
via `partial`/`unsupported`. Uses per-call auth. Preserve determinism and read-only.
