...
class AuthConfig(BaseModel):
    ...
    aws_access_key_id_env: str = Field(
        default="AWS_ACCESS_KEY_ID", description="AWS access key ID env var"
    )
    aws_secret_access_key: Optional[str] = Field(None, description="AWS secret access key")
    aws_secret_access_key_env: str = Field(
        default="AWS_SECRET_ACCESS_KEY", description="AWS secret access key env var"
    )
    ...
    oauth2_client_id_env: Optional[str] = Field(
        None, description="Env var name for OAuth2 client ID"
    )
    oauth2_client_secret: Optional[str] = Field(None, description="OAuth2 client secret")
    oauth2_client_secret_env: Optional[str] = Field(
        None, description="Env var name for OAuth2 client secret"
    )
    ...
    oauth2_refresh_token_env: Optional[str] = Field(
        None, description="Env var name for OAuth2 refresh token"
    )
    ...
    def get_aws_credentials(self) -> Dict[str, Optional[str]]:
        ...
        return {
            ...
            "aws_secret_access_key": os.environ.get(
                self.aws_secret_access_key_env, self.aws_secret_access_key
            ),
    ...
    def get_oauth2_credentials(self) -> Dict[str, Optional[str]]:
        ...
        return {
            "client_id": os.environ.get(self.oauth2_client_id_env, self.oauth2_client_id)
            if self.oauth2_client_id_env
            else self.oauth2_client_id,
            "client_secret": os.environ.get(
                self.oauth2_client_secret_env, self.oauth2_client_secret
            )
            if self.oauth2_client_secret_env
            else self.oauth2_client_secret,
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if d.get("oauth2_client_secret") and len(d.get("oauth2_client_secret", "")) > 8:
            d["oauth2_client_secret"] = (
                d["oauth2_client_secret"][:4] + "****" + d["oauth2_client_secret"][-4:]
            )
