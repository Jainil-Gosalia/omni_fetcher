...
async def s3_direct_credentials_example():
    ...
    print(f"  AWS Access Key ID: {fetcher.aws_access_key_id[:8]}..." if fetcher.aws_access_key_id else "  AWS Access Key ID: None")
...
async def s3_auth_config_example():
    ...
    print(f"  Access Key ID: {auth.get_aws_credentials().get('aws_access_key_id', 'None')[:8]}..." if auth.get_aws_credentials().get('aws_access_key_id') else "  Access Key ID: None")
...
async def s3_env_loading_example():
    ...
    # Clean up
    for key in ["OMNI_S3_TYPE", "OMNI_S3_AWS_ACCESS_KEY_ID_ENV",
                "OMNI_S3_AWS_SECRET_ACCESS_KEY_ENV", "OMNI_S3_AWS_REGION",
